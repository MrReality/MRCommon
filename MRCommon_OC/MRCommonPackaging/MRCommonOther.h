...
/// 返回网络状态
typedef void(^CommonNetWorkBlock)(CommonOtherNetWorkType type);
...
/**
 1 判断网络状态  To determine the network status
...
/**
 4 判断号码状态 Determine number state
...
/**
 6 获取 根控制器 To obtain the root controller
 */
+ (UIViewController *)getRootVC;

/**
 7 获取 view 所在的控制器 Get a view of the controller
 @param view 视图
 */
+ (UIViewController *)getViewControllerByView:(UIView *)view;

/**
 8 获取文件大小 Access to the file size
 */
+ (long long)fileSizeAtPath:(NSString *)filePath;

/**
 *  9 获取 path 路径文件夹的大小                Get the size of the folder path path
...
/**
 *  10 清除 path 路径文件夹的缓存            To clear the path cache folder
...
/**
 11 MD5 32 位加密 MD5 32-bit encryption
 */
...
/**
 13 添加偏移量的加密 Add the offset of the encryption
 */
+ (NSString *)MD5WithPassWord:(NSString *)str;

/**
  14 创建一个 两选项的 alert, Create a two options alert
...
/**
15 创建一个 单选项的 alert, Create a one option alert
 */
...
/// 17 跳转到 iphone 指定页面, 需要在 info 添加 URL types 字段, 并在 item0 里添加 URL Schemes 值为 prefs
+ (void)jumpWithType:(CommonOtherJumpType)type;

/// 19 根据路径创建文件 第二个参数, 是否覆盖掉
+ (BOOL)creatFileWithPath:(NSString *)path isCover:(BOOL)isCover;

/// 20 求两线的交点坐标  ab 一条线, cd 一条线
