...
- (void)layoutSubviews{
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        [label sizeToFit];
        if(label.mr_width > width){
            width = label.mr_width;
        }
    }
    ...
    self.sharpImgView.mr_x = self.beginPoint.x - 4;
    self.sharpImgView.mr_y = self.beginPoint.y;
    // 下拉栏起始点的位置
    __block CGPoint buttonPoint;
    // 判断初始点的位置, 确定动画从左往右还是从右往左
    if(self.beginPoint.x < kScreenWidth / 2){           // 动画从左往右
        buttonPoint = CGPointMake(self.sharpImgView.mr_x - kSkewing, self.beginPoint.y + self.sharpImgView.mr_height);
    }

    if(self.beginPoint.x > kScreenWidth / 2){           // 动画从右往左
        buttonPoint = CGPointMake(self.sharpImgView.mr_x + kSkewing + self.sharpImgView.mr_width, self.beginPoint.y + self.sharpImgView.mr_height);
    }
    
    self.buttonView.frame = CGRectMake(buttonPoint.x, buttonPoint.y, 0, 0);
    self.buttonView.layer.cornerRadius = cornerRadius;
//    self.buttonView.layer.borderColor = [UIColor blackColor].CGColor;
//    self.buttonView.layer.borderWidth = .5;
    ...
    for (NSInteger i = 0; i < count; i++) {
        [UIView animateWithDuration:duration animations:^{
            ...
            if(self.beginPoint.x > kScreenWidth / 2){           // 动画从右往左
                self.buttonView.mr_x = buttonPoint.x - width;
            }
            self.buttonView.mr_width = width;
            self.buttonView.mr_height = height * (i + 1);
            ...
            if(self.imageArray.count){
                ...
                imageView.mr_width = imageWidth;
                imageView.mr_height = imageWidth;
                imageView.mr_y = height / 2 - imageWidth / 2 + i * height;
                if(!self.imageType){                // 图片在左边
                    imageView.mr_x = kSpace;
                }else{                              // 图片在右边
                    imageView.mr_x = width - kSpace - imageWidth;
                }
...
- (UIImageView *)sharpImgView{
    ...
    if(!_sharpImgView){
        ...
        _sharpImgView.mr_size = CGSizeMake(8, 6);
