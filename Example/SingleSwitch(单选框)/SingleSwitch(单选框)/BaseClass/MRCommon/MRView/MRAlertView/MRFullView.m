...
- (void)setContent {
    ...
    _weekLabel.left = _dateLabel.left + _dateLabel.top + 10;
    _weekLabel.centerY = _dateLabel.centerY;
    
    _closeButton.size = CGSizeMake([UIScreen mainScreen].bounds.size.width, 44);
    _closeButton.top = [UIScreen mainScreen].bounds.size.height - _closeButton.height;
...
- (void)commonInitialization {
    ...
    _scrollContainer.size = CGSizeMake([UIScreen mainScreen].bounds.size.width, _itemSize.height * kRows + _gap  + 150);
    _scrollContainer.top = [UIScreen mainScreen].bounds.size.height - _closeButton.height - _scrollContainer.height;
    _scrollContainer.contentSize = CGSizeMake(kPages * [UIScreen mainScreen].bounds.size.width, _scrollContainer.height);
    ...
    for (NSInteger i = 0; i < kPages; i++) {
        ...
        pageView.left = i * [UIScreen mainScreen].bounds.size.width;
        ...
        for (NSInteger i = 0; i < kRows * kRow_count; i++) {
            ...
            if (item.tag < models.count) {
                ...
                [item updateLayoutBySize:_itemSize finished:^(MRLabel *item) {
                    ...
                    item.top = (_itemSize.height + _gap) * v + _gap + 100;
                }];
            }
        }
    }];
