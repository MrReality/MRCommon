...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    self.promptView.size = CGSizeMake(0, 0);
...
- (void)layoutSubviews{
    ...
    self.titleLabel.left = 2 * kSpace;
    
    if(_showImage){         /// 有显示图片
        self.showImgView.left = 2 * kSpace;
        self.showImgView.top = self.titleLabel.bottom + kSpace / 2;
        self.showImgView.image = self.showImage;
        
        self.textField.frame = CGRectMake(self.showImgView.width + 4 * kSpace, self.showImgView.top, self.width - 6 * kSpace - self.showImgView.width, 35);
        self.textField.borderStyle = UITextBorderStyleNone;
        self.lineView.frame = CGRectMake(self.textField.left, self.textField.top + self.textField.height - 3, self.textField.width, 1);
    }
    
    if(!_showImage){            /// 没有显示图片
        self.textField.frame = CGRectMake(2 * kSpace, self.titleLabel.top + self.titleLabel.height + kSpace / 2, self.width - 4 * kSpace, 35);
    }
