...
/// MRDropDownView 被点击调用的 delegate, 只有筛选器被点开时才调用 MRDropDownView clicked invoke the delegate, only call filter was clicked on
- (void)viewDidTap;

/// 点击选项调用的方法, 不要用下面那个 block, 会内存泄漏 Click on the option method call
