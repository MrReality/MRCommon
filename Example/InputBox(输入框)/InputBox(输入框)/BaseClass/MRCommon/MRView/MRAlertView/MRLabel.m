...
- (void)setModel:(MRIconLabelModel *)model {
    _textLabel.text = model.text;
    _iconView.image = model.icon;
}

- (void)updateLayoutBySize:(CGSize)size finished:(void (^)(MRLabel *))finished {
    CGRect frame = self.frame;
    frame.size = size;
    self.frame = frame;
...
@implementation MRIconLabelModel

+ (instancetype)modelWithTitle:(NSString *)title image:(UIImage *)image {
    
    MRIconLabelModel *model = [[MRIconLabelModel alloc] init];
    model.text = title;
    model.icon = image;
    return model;
}
